...
author: "Lucas Schiffer, Marcel Ramos"
...
# Mean observed difference p-value
mean.p <- (sum(abs(avgdiff) > abs(obsdiff)) + 1) / (length(avgdiff) + 1)
```
...
hist(avgdiff2)
abline(v = obs2, col = "darkgreen", lwd = 2)
# Median observed difference p-value
median.p <- (sum(abs(avgdiff2) > abs(obs2)) + 1) / (length(avgdiff2) + 1)
```
